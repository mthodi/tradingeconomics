<template>
  <v-container fluid>
    ...
    <div v-if="values">
      <!-- <p class="bg-success mb-0 text-center">
               Click on the index name to get more details about the index.</em></p> -->
      <v-toolbar flat color="white">
        <v-toolbar-title>Market Indices</v-toolbar-title>
      </v-toolbar>
      <v-data-table :headers="columns" :items="values" class="elavation-1">
        <template v-slot:items="props">
            ...
            <!-- <td :class="[( props.item.WeeklyPercentualChange > 0 ? 'text-xs-justify green--text' : 'text-s-justify red--text')]">{{ props.item.WeeklyPercentualChange}}</td> -->
